...
def load_pdfs_from_folder(folder_path: str):
    ...
    curated_path = os.path.join(folder_path, "curated_catia_facts.md")
    if os.path.exists(curated_path):
        curated_text = open(curated_path, encoding="utf-8").read()
        all_pages.append(Document(
            page_content=curated_text,
            metadata={"source": curated_path, "source_file": "curated_catia_facts.md", "lang": "ko", "curated": True},
        ))
    if not all_pages:
