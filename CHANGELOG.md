...
### Added — Streamlit UI/UX 개선
- **답변 유지**: `st.session_state`에 QA/절차검증 결과를 저장 -- 사이드바 Top-K 등 다른 위젯을 조작해서 스크립트가 재실행돼도 방금 받은 답변이 사라지지 않게 함
- **응답 시간 표시**: 질문/절차검증 각각 소요 시간을 측정해 `⏱️ 응답 시간: N.NN초`로 표시
- **사이드바 통계 동적화**: `get_corpus_stats()` 추가 -- `pypdf`로 실제 PDF 페이지 수, Chroma 컬렉션에서 실제 청크 수를 집계해서 표시. 하드코딩된 수치(청킹 로직 바뀔 때마다 실제 값과 어긋나던 문제) 제거
- **입력 커밋 안정성**: 질문/절차검증 입력 폼을 `st.form()`으로 감싸서, 텍스트 입력 후 Enter 없이 바로 제출 버튼을 눌러도 값이 확실히 전달되도록 수정

### Changed — 디자인
- `.streamlit/config.toml` 추가 -- 화이트 배경 + 네이비블루(`#1E3A8A`) 포인트 테마로 버튼/슬라이더 등 기본 위젯 색상 통일
- `src/app.py` CSS 리뉴얼: 메인 헤더 밑줄 포인트, 배지(Strict/Adaptive/Off) 알약형 디자인, 사이드바 배경 분리, 버튼 호버 효과, 탭/expander/지표 카드 모양 정리
